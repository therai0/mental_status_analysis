...
from src.components.data_transformation.transformation.transformation import DataTransformation
...
class ELT_pipeline:
    ...
    def init_ELT(self):
        try:
            ...
            train_arr_path,test_arr_path,target_train_arr_path,target_test_arr_path = data_transformation.init_data_transormation()

            logging.info("Initation of model traning")
            model_traning = ModelTrain(
                train_arr_path,
                test_arr_path,
                target_train_arr_path,
                target_test_arr_path
                )
            model_evl_train,model_evl_test = model_traning.init_model_traning()
            logging.info(model_evl_train)
            logging.info(model_evl_test)
